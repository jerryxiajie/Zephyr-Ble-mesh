...
static struct device *gpio,*gpio_led;
...
static void configure_button(void)
{
	...
	gpio_led = device_get_binding(LED_ROW1_GPIO_PORT);

	gpio_pin_configure(gpio, SW0_GPIO_PIN,
			   (GPIO_DIR_IN | GPIO_INT | GPIO_INT_EDGE |
			    GPIO_INT_ACTIVE_LOW));

	gpio_pin_configure(gpio_led, LED_ROW1_GPIO_PIN,GPIO_DIR_OUT);

	gpio_pin_write(gpio_led,LED_ROW1_GPIO_PIN,0);
...
{
	gpio_pin_write(gpio_led,LED_ROW1_GPIO_PIN,1);
}
